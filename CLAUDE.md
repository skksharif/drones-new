...
**Home banners.** `banners` is the third collection. It is an all-or-nothing switch: while any banner has `active: true` the home carousel shows only those, and with none active it falls back to the automatic slides built from live offers. The home page filters out inactive banners *before* passing them to `PromoBanner` — that component is a client component, so a parked draft's headline would otherwise be readable in the page source. A banner's `href` is validated as a same-site path; it is the most prominent link on the site and must not be able to point elsewhere.
